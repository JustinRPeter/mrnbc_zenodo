...
c&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
 	call QM_dm(rec,gcmc,gcmf,ns,nsgc,nycur,ngcur,nvar,
...
c	apply final correction and store final corrected series
	...
	enddo
	enddo
c	transfer current climate bias corercted value to the raw series
618	continue
	if(jj.gt.1)then
 	do i=1,ngcur
	do j=1,nout
...
c	following is the end of do loop for itr
	enddo
c	finally re-apply Qm at daily level to match the daily distribution
c**********************************************
c&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
	do jj=1,2
	if(jj.eq.1)then
...
c***********************************************************
	SUBROUTINE qm_bias(xx,yy,zz,N,NNMAX,avh,avr)
...
c	assign original ranks
	...
	END
c***************************************************************
	subroutine QM_DM(rec,gcmc,gcmf,ns,nsgc,nycur,ngcur,nvar,
     1                itime,ngfut,nsgf,leap,idays,nout,phlwr,phupr,
     2                avh,avr,nvarmax,nyrmax,monmax,ndmax,nnmax)
	real*4 rec(nvarmax,nyrmax,monmax,ndmax),
	1	   gcmc(nvarmax,nyrmax,monmax,ndmax),
	2       gcmf(nvarmax,nyrmax,monmax,ndmax)
	REAL*4 avh(nvarmax),avr(nvarmax)
	real*4 xx(nnmax),yy(nnmax),zz(nnmax)
...
c	call quantile matching bias correction
	kk=min(kkh,kkc,kkf)
	call qm_bias(xx,yy,zz,kk,nnmax,avh(il),avr(il))
...
32	continue
	...
	enddo
c$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
c	to deal with negatives and to keep the mass balance, check totals and aply a CORRECTION
...
c	check the values to maintain number of zeros	
	ab=simc(i,l)
c	if(ilimit(il).gt.0)then
c	if(gcmc(il,i,j,l).lt.thres(il).and.ab.ge.thres(il))
c	1                 ab=gcmc(il,i,j,l)
c	if(gcmc(il,i,j,l).ge.thres(il).and.ab.lt.thres(il))
c	1                 ab=thres(il)
c	endif
...
c	maintain mass balance
	...
	do l=1,jd
	if(sim2.ne.0.0)gcmc(il,i,j,l)=simc(i,l)*sim1/sim2
...
c	check the values to maintain number of zeros	
	ab=simf(i,l)
c	if(ilimit(il).gt.0)then
c	if(gcmf(il,i,j,l).lt.thres(il).and.ab.ge.thres(il))
c	1                 ab=gcmf(il,i,j,l)
c	if(gcmf(il,i,j,l).ge.thres(il).and.ab.lt.thres(il))
c	1                 ab=thres(il)
c	endif
 	if(ab.lt.phlwr(il))ab=phlwr(il)
	if(ab.gt.phupr(il))ab=phupr(il)
...
c	maintain mass balance
	...
	do l=1,jd
	if(sim2.ne.0.0)gcmf(il,i,j,l)=gcmf(il,i,j,l)*sim1/sim2
